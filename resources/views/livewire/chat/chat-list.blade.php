...
x-init="$nextTick(() => {
    ...
    // Scroll to the selected chat
    if (conversationElement) {
        conversationElement.scrollIntoView({ behavior: 'smooth' });
    }
})"
class="flex flex-col transition-all h-full w-full overflow-hidden p-2">
    <header class="p-4 z-10 bg-white sticky top-0 w-full">
        <div class="justify-between flex items-center pb-2">
            ...
            <button class="">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5h18M6 10h12M9 15h6" />
                </svg>
            </button>
        </div>
        ...
        <div class="flex gap-3 items-center py-2 bg-white">
            ...
            <button @click="type='all'" :class="{ 'bg-pink-100 border-0': type == 'all' }"
                class="text-[#f541c2] font-semibold inline-flex justify-center items-center rounded-full gap-x-1 text-[14px] px-2 lg:px-4 py-1  lg:py-2 border ">
                All
            </button>
            <button @click="type='deleted'" :class="{ 'bg-pink-100 border-0': type == 'deleted' }"
                class="text-[#f541c2] font-semibold inline-flex justify-center items-center rounded-full gap-x-1 text-[14px] px-2 lg:px-4 py-1  lg:py-2 border ">
                Deleted
            </button>
            ...
            @if ($conversations)
                ...
                <li id="conversation-{{ $conversation->id }}"
                    ...
                    <a href="#" class="shrink-0">
                        <x-avatar src="https://randomuser.me/api/portraits/men/{{ $conversation->getReceiver()->id }}.jpg" />
                    </a>
                    ...
                    <aside class="grid grid-cols-12 w-full">
                        ...
                        <a href="{{ route('chat', $conversation->id) }}"
                            ...
                            <div class="flex justify-between w-full items-center">
                                ...
                                @if ($conversation->messages->last())
                                <small class="text-gray-700">{{$conversation->messages?->last()?->created_at?->shortAbsoluteDiffForHumans()}}<span> ago</span> </small>
                                @endif
                            </div>
                            ...
                            <div class="flex gap-x-2 items-center">
                                ...
                                <span
                                    class="font-bold p-px px-2 text-xs shrink-0 rounded-full bg-[#FF61D2] text-white">
